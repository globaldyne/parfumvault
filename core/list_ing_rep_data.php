...
if($_POST['view'] == 'ingredients'){
	...
	$q = mysqli_query($conn,"SELECT id,ing_rep_name,ing_rep_cas,notes FROM ingReplacements WHERE ing_name = '$ing_name'");
	if (mysqli_num_rows($q)==0) { 
		$q = mysqli_query($conn,"SELECT id,ing_name,ing_cas,notes FROM ingReplacements WHERE ing_rep_name = '$ing_name'");
	}
...
if($_POST['view'] == 'formula'){
	...
	while($res = mysqli_fetch_array($q)){
		...
		$q2 = mysqli_query($conn, "SELECT id,ing_name,ing_rep_name,ing_rep_cas,notes FROM ingReplacements WHERE ing_name = '".$res['ingredient']."'");
