...
<div id="addOrderForm">
    ...
    <div class="table-responsive">
        <table id="orderItemsTable" class="table table-hover">
            <thead>
                <tr>
                    <th>Item <i class="fa fa-info-circle" data-bs-toggle="tooltip" data-bs-placement="top" title="Select the item from the supplier's list."></i></th>
                    ...
                    <th>Lot Number <i class="fa fa-info-circle" data-bs-toggle="tooltip" data-bs-placement="top" title="Optional. A lot number is a unique identifier assigned to a batch of products."></i></th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
...
<script>
    $(document).ready(function() {
        ...
        function populateItemSelect() {
            ...
            $.getJSON('/core/list_suppliers_materials_data.php?supplier_id=' + supplierId, function(response) {
                ...
                if (!Array.isArray(items)) {
                    ...
                    deferred.resolve('<select class="form-control" name="item[]"><option value="">Error loading items</option></select>');
                } else {
                    ...
                    let options = items
                        ...
                        .map(item => `<option value="${item.material}" data-price="${item.price}" data-size="${item.size}" data-sku="${item.supplier_sku || ''}">${item.material} (CAS: ${item.cas})</option>`)
                        .join('');
                    if (options === '') {
                    ...
                    deferred.resolve(`<select class="form-control" name="item[]">${options}</select>`);
                }
            });
        ...
        function updateTotal() {
            ...
            $('#totals').text(selectedCurrency + ' ' + total.toFixed(2));
        }
        ...
        $('#addItem').click(function() {
            populateItemSelect().done(function(itemSelect) {
                ...
                // Add event listener to populate unit price, size, subtotal, and lot number when item is selected
                $(newRow).find('select[name="item[]"]').change(function() {
                    var selectedOption = $(this).find('option:selected');
                    var unitPrice = selectedOption.data('price');
                    var size = selectedOption.data('size');
                    var sku = selectedOption.data('sku');
                    var quantity = parseFloat($(newRow).find('input[name="quantity[]"]').val()) || 1;
                    var subtotal = unitPrice * quantity;
                    $(newRow).find('input[name="unit_price[]"]').val(unitPrice);
                    $(newRow).find('input[name="size[]"]').val(size);
                    $(newRow).find('input[name="subtotal[]"]').val(subtotal.toFixed(2));
                    $(newRow).find('input[name="lot_number[]"]').val(sku);
                    updateTotal();
                });
        ...
        $('#saveNewOrder').click(function() {
            ...
            $('#orderItemsTable tbody tr').each(function() {
                ...
                if (!item.item || !item.size || !item.unit_price || !item.quantity) {
                    hasEmptyFields = true;
                    row.find('select[name="item[]"], input[name="size[]"], input[name="unit_price[]"], input[name="quantity[]"]').each(function() {
                        if (!$(this).val()) {
            ...
            if (!supplierId || !currency || hasEmptyFields) {
                $('#addOrderModalMsg').html('<div class="alert alert-danger alert-dismissible fade show mx-3 mt-3" role="alert"><i class="bi bi-exclamation-circle mx-2"></i>Please fill in all required fields.<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                return;
            }
        ...
        $('#orderItemsTable tbody').on('input', 'select[name="item[]"], input[name="size[]"], input[name="unit_price[]"], input[name="quantity[]"]', function() {
            if ($(this).val()) {
                $(this).removeClass('is-invalid');
            }
        });
