    ...
    $('#saveOrderChanges').click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    $('#updateOrderModalMsg').html('<div class="alert alert-success"><i class="bi bi-check-circle mx-2"></i>' + data.success + '</div>');
                    $('#tdOrdersData').DataTable().ajax.reload(null, true);
                } else {
                    $('#updateOrderModalMsg').html('<div class="alert alert-danger"><i class="bi bi-exclamation-circle mx-2"></i>' + data.error + '</div>');
                }
            },
            error: function(xhr, status, error) {
                $('#updateOrderModalMsg').html('<div class="alert alert-danger"><i class="bi bi-exclamation-circle mx-2"></i>An error occurred while updating the order. ' + error + '</div>');
            }
        });
    });
});
