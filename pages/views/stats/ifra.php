...
if(!mysqli_num_rows(mysqli_query($conn, "SELECT id FROM IFRALibrary WHERE owner_id = '$userID'"))){
	echo '<div class="alert alert-info"><strong>INFO: </strong> You need to <a href="index.php?do=IFRA">import</a> the IFRA xls first.</div>';
	return;
}
?>
<script>
var chart = AmCharts.makeChart("chartIFRA",{
  ...
  "dataProvider"  : [
<?php
$ifratypes = mysqli_query($conn, "SELECT DISTINCT type FROM IFRALibrary WHERE owner_id = '$userID'");
