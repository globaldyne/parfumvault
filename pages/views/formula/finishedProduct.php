    ...
    <div class="container-fluid">
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                ...
                <p>*Values in: <strong class="alert alert-danger mx-2">red</strong> exceeds usage level, <strong
                        class="alert bg-banned mx-2">dark red</strong> banned/prohibited, <strong
                        class="alert alert-warning mx-2">yellow</strong> Specification,<strong
                        class="alert alert-success mx-2">green</strong> are within usage level,<strong
                        class="alert alert-info mx-2">blue</strong> are exceeding recommended usage level</p>
...
<div class="modal fade" id="ViewBoxLabel" data-bs-backdrop="static" tabindex="-1" role="dialog"
    aria-labelledby="ViewBoxLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="printLabel"
                    data-print-target=".modal-Label-body-data">Print<i
                        class="fa-solid fa-arrow-up-right-from-square ml-2"></i></button>
            </div>
        </div>
    </div>
</div>
...
<!-- Modal IFRA DOC-->
<div class="modal fade" id="IFRA" data-bs-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="IFRA"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="alert alert-info"><i class="fa-solid fa-circle-exclamation mr-2"></i>IMPORTANT: The
                    generated document isn't an official IFRA certificate and needs to be reviewed by a certified
                    person. Also, data needs to be properly verified to make sure there are no errors.</div>
    ...
    <!-- Modal IFRA Doc -->
    <div class="modal fade" id="ifraDocModal" data-bs-backdrop="static" tabindex="-1" role="dialog"
        aria-labelledby="ifraDocModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable modal-fullscreen" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" id="printIFRADoc"
                        data-print-target=".modal-IFRA-body-data">
    ...
    $(document).ready(function() {
        ...
        var formula_table = $('#formula').DataTable({
            ...
            footerCallback: function(tfoot, data, start, end, display) {
                ...
                if (response) {
                    ...
                    $('#bottle-quantity').text(response.meta['bottle_quantity'] + response.meta[
                        'quantity_unit']);
                    $('#accessory-cost').html(response.meta['currency'] + response.meta[
                        'accessory_cost']);
                    $('#accessory').text(response.meta['accessory']);

                    if (response.meta['batchNo']) {
                        $('#batch-no').html('<a href="/pages/viewDoc.php?type=batch&id=' + response
                            .meta['batchNo'] + '" target="_blank">' + response.meta['batchNo'] +
                            '</a>');
                    } else {
                        ...
                        switch (restriction) {
                            case 1:
                                setAlertClassAndIcon(selector, 'alert-info',
                                    `Recommended usage: ${limit}% <p>PV Regulated</p>`);
                                break;
                            case 2:
                                setAlertClassAndIcon(selector, 'alert-danger',
                                    `Restricted usage: ${limit}% <p>PV Regulated</p>`);
                                break;
                            case 3:
                                setAlertClassAndIcon(selector, 'alert-warning',
                                    `Specification: ${limit}% <p>PV Regulated</p>`);
                                break;
                            case 4:
                                setAlertClassAndIcon(selector, 'alert-warning',
                                    `Prohibited or Banned - <p>PV Regulated</p>`);
                                break;
                            default:
            ...
            var m = message.split(' - ');
            $('#results').html(
                '<div class="alert alert-danger"><i class="fa-solid fa-circle-exclamation mx-2"></i><strong>' +
                m[1] + '</strong></div>');
        });
        ...
        function createAlertBox(response) {
            ...
            if (response.compliance.inval_materials.data && response.compliance.inval_materials.data.length >
                0) {
                alertBox +=
                    '<br><a data-bs-toggle="collapse" href="#invalidMaterials" role="button" aria-expanded="false" aria-controls="invalidMaterials">' +
                    'Show details</a>' +
        ...
        $("#ViewBoxLabel").on("show.bs.modal", function(e) {
            ...
            const url = "/core/core.php?action=" + action + "&batchID=" + batchNo + "&fid=" + fid +
                "&carrier=" + carrier_concentration + "&download=text";
        ...
        $('#generateDoc').click(function() {
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    data = '<div class="alert alert-danger">Unable to generate data, ' +
                        error + '</div>';
                    $('.modal-IFRA-body-data').html(data);
                    $('#ifraDocModal').modal('show');
                    $("#generateDoc").prop("disabled", false);
