...
$ingUsage = mysqli_query($conn,"SELECT name,fid FROM formulas WHERE ingredient = '".$ingName."'");
while($used_res = mysqli_fetch_array($ingUsage)){
	$used[] = $used_res;
}
$ingUsageCmp = mysqli_query($conn,"SELECT ing,min_percentage,max_percentage FROM ingredient_compounds WHERE name = '".$ingName."'");
