...
function checkVer($app_ver, $db_ver) {
	if($app_ver){
		...
		if($db_ver < $data){	
			echo '<div class="alert alert-warning alert-dismissible"><strong>Your database schema needs to be updated ('.$db_ver.'). Please <a href="pages/maintenance.php?do=backupDB">backup</a> your database first and then click <a href="javascript:updateDB()">here to update the db schema.</a></strong></div>';
		}
	}
}
?>
