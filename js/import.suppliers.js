...
$(".progress").hide();
$("#btnImportSuppliers").prop("disabled", true);
$("#jsonFile").change(function(){
	...
	$("#btnImportSuppliers").prop("disabled", false);
	$('#btnImportSuppliers').prop('value', 'Import');
});


$('#btnImportSuppliers').click(function() {
	...
	$.ajax({ 
		url: '/core/core.php?action=importSuppliers', 
		type: 'POST',
		data: fd,
		contentType: false,
      	processData: false,
		...
		xhr: function () {
			...
			$(".progress").show();
			$("#btnImportSuppliers").prop("disabled", true);
			$('#btnImportSuppliers').prop('value', 'Please wait...');
        	return xhr;
        },	
