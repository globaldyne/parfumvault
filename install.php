	...
	if(strlen($_POST['password']) < '5'){
		...
		if($_POST['dbhost'] && $_POST['dbuser'] && $_POST['dbpass'] && $_POST['dbname'] && $_POST['username'] && $_POST['password'] && $_POST['fullName'] && $_POST['email']){
			...
			if(!$link = mysqli_connect($_POST['dbhost'], $_POST['dbuser'], $_POST['dbpass'], $_POST['dbname'])){
				$msg = '<div class="alert alert-danger alert-dismissible">Error connecting to the database, make sure the details provided are correct, the database exists and the user has write permissions.</div>';
			}else{
				$cmd = 'mysql -u'.$_POST['dbuser'].' -p'.$_POST['dbpass'].' -h'.$_POST['dbhost'].' '.$_POST['dbname'].' < ./db/pvault.sql'; 
				passthru($cmd,$e);
				if(!$e){
					mysqli_query($link,"INSERT INTO users (username,password,fullName,email) VALUES ('".strtolower($_POST['username'])."',PASSWORD('".$_POST['password']."'),'".$_POST['fullName']."','".$_POST['email']."')");
