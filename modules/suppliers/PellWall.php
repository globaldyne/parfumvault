...
$q = $_GET['q'];
$url = 'https://pellwall.com';
$uri = '?s='.$q.'&post_type=product';
...
foreach(myDOM($url,$uri)->query('//li') as $element){
	...
	foreach($links as $a) {
		foreach(myDOM($url,$a->getAttribute('href'))->query('//div[@class="products elements-grid align-items-start woodmart-products-holder  woodmart-spacing-30 pagination-more-btn row grid-columns-4"]|//a[@class="btn-doc btn-doc-default"] ') as $element2){
